...
bool initGame(jadel::Window *window)
{
    ...
    if (!loadBMP("res/clutter.bmp", &currentGame->clutterSprite))
        return false;
    if (!loadBMP("res/grass.bmp", &currentGame->walkSurface))
        return false;
    if (!loadBMP("res/dude1.bmp", &currentGame->playerSprite))
        return false;
    if (!loadBMP("res/dude1l.bmp", &currentGame->playerSpriteLeft))
        return false;
    if (!loadBMP("res/wall.bmp", &currentGame->wallSurface))
        return false;
    if (!loadBMP("res/poison.bmp", &currentGame->poisonSprite))
        return false;
    if (!loadBMP("res/hpotion.bmp", &currentGame->hpackSprite))
        return false;
    if (!loadBMP("res/portal.bmp", &currentGame->portalSprite))
        return false;
    ...
    if (jadel::inputIsKeyTyped(jadel::KEY_M))
    {
        ...
        if (portal)
        {
            ...
            for (int i = 0; i < targetWorld->numPortals; ++i)
            {
                ...
                if (targetPortal->worldLinkID == currentGame->currentWorld->entity.GUID && targetPortal->linkID == portal->linkID)
                {
        ...
        if (currentGame->screenPos.x > -screenTilemapW && currentGame->screenPos.y > -screenTilemapH && currentGame->screenPos.x < currentGame->currentWorld->width && currentGame->screenPos.y < currentGame->currentWorld->height)
        {
            ...
            for (int y = yStart; y < yEnd; ++y)
            {
                for (int x = xStart; x < xEnd; ++x)
                {
                    ...
                    jadel::Recti entityDim = {.x = sectorPos.x,
                                              .y = sectorPos.y,
                                              .w = currentGame->tileScreenW,
                                              .h = currentGame->tileScreenH};
